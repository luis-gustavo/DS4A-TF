...
```{r}
json.geociencias.perfil <- 'data/geociencias.profile.json'
file.info(json.geociencias.perfil)
```
...
```
Os arquivos `r json.geografia.perfil`, `r json.geociencias.perfil` e `r json.geologia.perfil` apresentam dados sobre o perfil de todos os docentes vinculados a programas de pós-graduação de Geografia, Geociências e Geologia da UnB, entre 2010 e 2017.
...
```{r}
json.geociencias.advise <- "data/geociencias.advise.json"
file.info(json.geociencias.advise)
```
...
Os arquivos `r json.geografia.advise`, `r json.geociencias.advise` e `r json.geologia.advise` apresentam dados sobre o orientações de mestrado e doutorado feitas por todos os docentes vinculados a programas de pós-graduação de Geografia, Geociências e Geologia da UnB, entre 2010 e 2017.
...
```{r}
json.geociencias.producao.bibliografica <- "data/geociencias.publication.json"
file.info(json.geociencias.producao.bibliografica) 
```
...
Os arquivos `r json.geografia.producao.bibliografica`, `r json.geociencias.producao.bibliografica` e `r json.geologia.producao.bibliografica` apresentam dados sobre a produção bibliográfica gerada por todos os docentes vinculados a programas de pós-graduação de Geografia, Geociências e Geologia da UnB, entre 2010 e 2017.
...
```{r}
json.geociencias.researchers_by_area <- "data/geociencias.researchers_by_area.json" 
file.info(json.geociencias.researchers_by_area)
```
...
Os arquivos `r json.geografia.researchers_by_area`, `r json.geociencias.researchers_by_area` e `r json.geologia.researchers_by_area` apresentam as vinculações de todos os docentes de Geografia, Geociências e Geologia que declararam atuar em cada uma das áreas de pós-graduação do Sistema Nacional de Pós-Graduação da CAPES, conforme apresenta-se registrada essa informação no currículo Lattes de cada um, em data recente.


### Redes de colaboração entre docentes

```{r}
json.geografia.redes <- "data/Geografia.graph.json" 
file.info(json.geografia.redes)
```

```{r}
json.geociencias.redes <- "data/geociencias.graph.json" 
file.info(json.geociencias.redes)
```

```{r}
json.geologia.redes <- "data/geologia.graph.json" 
file.info(json.geologia.redes)
```

Os arquivos `r json.geografia.redes`, `r json.geociencias.redes` e `r json.geologia.redes` apresentam redes de colaboração na co-autoria de artigos científicos, feitas entre os docentes vinculados a programas de pós-graduação de Geografia, Geociências e Geologia da UnB, entre 2010 e 2017.
...
Os arquivos Geografia.profile.json, geociencias.profile.json e geologia.profile.json contém dados que caracterizam o perfil profissional de cada um dos docentes do grupo sob análise. Esse arquivo pode ser lido através do seguinte comando, da biblioteca _jsonlite_.
...
```{r}
unb.geociencias.prof <- fromJSON("data/geociencias.profile.json")
```

```{r}
unb.geologia.prof <- fromJSON("data/geologia.profile.json")
```

A quantidade de docentes de Geografia, Geociências e Geologia sob análise é apresentada a seguir.
...
```{r}
length(unb.geociencias.prof)
```

```{r}
length(unb.geologia.prof)
```
...
```{r}
names(unb.geociencias.prof[[1]])
```

```{r}
names(unb.geologia.prof[[1]])
```

Ainda com o intuito de obter uma análise inicial dos dados contidos nos perfis dos docentes, pode-se usar a função _glimpse()_,  que apresenta os atributos típicos que podem ser obtidos relativamente a um pesquisador específico.
...
```{r}
unb.geociencias.adv <- fromJSON("data/geociencias.advise.json")
```

```{r}
unb.geologia.adv <- fromJSON("data/geologia.advise.json")
```
...
Explorando os dados da Geografia, de orientações concluídas de mestrado e doutorados, no ano de 2017, percebe-se que existem 14 e 17 orientações, respectivamente. Dentre as de mestrado apenas uma não era do curso de geografia. E dentre as de doutorado, apenas duas não são do curso de geografia.

Nos dados da Geociências, existem 10 de mestrado e 19 de doutorado.

Nos dados da Geologia, existem 14 de mestrado e 27 de doutorado.
...
```{r}
length(unb.geociencias.adv$ORIENTACAO_CONCLUIDA_DOUTORADO$`2017`$natureza)
```

```{r}
length(unb.geologia.adv$ORIENTACAO_CONCLUIDA_DOUTORADO$`2017`$natureza)
```
...
```{r}
sort(table(unb.geociencias.adv$ORIENTACAO_CONCLUIDA_DOUTORADO$`2017`$curso), decreasing = TRUE)
```

```{r}
sort(table(unb.geologia.adv$ORIENTACAO_CONCLUIDA_DOUTORADO$`2017`$curso), decreasing = TRUE)
```
...
```{r}
length(unb.geociencias.adv$ORIENTACAO_CONCLUIDA_MESTRADO$`2017`$natureza)
```

```{r}
length(unb.geologia.adv$ORIENTACAO_CONCLUIDA_MESTRADO$`2017`$natureza)
```
...
```{r}
sort(table(unb.geociencias.adv$ORIENTACAO_CONCLUIDA_MESTRADO$`2017`$curso), decreasing = TRUE)
```

```{r}
sort(table(unb.geologia.adv$ORIENTACAO_CONCLUIDA_MESTRADO$`2017`$curso), decreasing = TRUE)
```
...
É evidente que no programa de pós-graduação de geografia da UnB há mais publicações de Periódicos, Texto em Jornais, Livros e em Eventos. Além disso, percebe-se que artigos aceitos é de longe a menor representação, em termos quantitativos, das publicações desse programa.

#### Orientações
Foi analisado o arquivo Geologia.advise.json,que possui as orientações vinculadas aos perfis dos professores da Geologia, separadas em listas pelo tipo de orientação, sendo que cada tipo possui listas classificadas por ano.

```{r echo=FALSE}
unb.geologia.adv.tb <- tibble()
for (i in 1:length(unb.geologia.adv[[1]]))
  unb.geologia.adv.tb <- rbind(unb.geologia.adv.tb,
                   unb.geologia.adv$ORIENTACAO_EM_ANDAMENTO_DE_POS_DOUTORADO[[i]])
for (i in 1:length(unb.geologia.adv[[1]]))
  unb.geologia.adv.tb <- rbind(unb.geologia.adv.tb,
                   unb.geologia.adv$ORIENTACAO_EM_ANDAMENTO_DOUTORADO[[i]])
for (i in 1:length(unb.geologia.adv[[1]]))
  unb.geologia.adv.tb <- rbind(unb.geologia.adv.tb,
                   unb.geologia.adv$ORIENTACAO_EM_ANDAMENTO_MESTRADO[[i]])
for (i in 1:length(unb.geologia.adv[[1]]))
  unb.geologia.adv.tb <- rbind(unb.geologia.adv.tb,
                   unb.geologia.adv$ORIENTACAO_EM_ANDAMENTO_GRADUACAO[[i]])
for (i in 1:length(unb.geologia.adv[[1]]))
  unb.geologia.adv.tb <- rbind(unb.geologia.adv.tb,
                   unb.geologia.adv$ORIENTACAO_EM_ANDAMENTO_INICIACAO_CIENTIFICA[[i]])
for (i in 1:length(unb.geologia.adv[[1]]))
  unb.geologia.adv.tb <- rbind(unb.geologia.adv.tb,
                   unb.geologia.adv$ORIENTACAO_CONCLUIDA_POS_DOUTORADO[[i]])
for (i in 1:length(unb.geologia.adv[[1]]))
  unb.geologia.adv.tb <- rbind(unb.geologia.adv.tb,
                   unb.geologia.adv$ORIENTACAO_CONCLUIDA_DOUTORADO[[i]])
for (i in 1:length(unb.geologia.adv[[1]]))
  unb.geologia.adv.tb <- rbind(unb.geologia.adv.tb,
                   unb.geologia.adv$ORIENTACAO_CONCLUIDA_MESTRADO[[i]])
for (i in 1:length(unb.geologia.adv[[1]]))
  unb.geologia.adv.tb <- rbind(unb.geologia.adv.tb,
                   unb.geologia.adv$OUTRAS_ORIENTACOES_CONCLUIDAS[[i]])
```

Foi gerado o gráfico que relaciona o número de orientações por ano, relacionado à natureza destas:

```{r fig.width=10, orient.tb}
ggplot(unb.geologia.adv.tb, aes(x = ano, fill = natureza)) +
  geom_bar(stat="count") +
  labs(x = "Ano", y = "Orientações", title = "Orientação X Ano")
```

Pode-se observar que o número de orientações por ano manteve uma média maior que 100, desde o ano de 2012.

O gráfico a seguir mostra a relação entre as agências financiadoras de pesquisas ao longo dos anos:

```{r fig.width=10, echo=FALSE}
ggplot(unb.geologia.adv.tb, aes(x = agencia_financiadora, y = ano)) +
  geom_count() +
  labs(x = "", y = "", title = "") +
    coord_flip()
```

Observa-se forte influência da CAPES e do CNPq nas áreas de pesquisa, sendo estes grandes financiadores.

Para observar o nível de internacionalização das orientações, foi gerado o gráfico abaixo, o qual demonstra arazão entre as instituições de origem das orientações:

```{r fig.width=10, echo=FALSE}
ggplot(unb.geologia.adv.tb, aes(x = 1, fill = instituicao)) +
  geom_bar() +
  labs(x = "", y = "", fill = "Instituição", title = "Orientações X Instituições") +
  coord_polar(theta = "y")
```

A lista de diferentes instituições pode ser observada com o seguinte código abaixo. Por meio dela e do gráficomostrado anteriormente, pode-se ter uma estimativa do número de orientações com influência internacional.

```{r }
distinct(unb.geologia.adv.tb, instituicao)
```
